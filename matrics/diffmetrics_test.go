...
func TestFileOriginWhitespaceExcluded(t *testing.T) {
	...
	expected := "{\"File\":\"testdata/file.txt\",\"Insertions\":3,\"Deletions\":0,\"LinesBefore\":0,\"LinesAfter\":3,\"NewFile\":true,\"DeleteFile\":false}"
	if string(got) != expected {
		t.Errorf("DiffMetrics expected %q but got %q", expected, got)
	}
}
...
func TestFileAddOnlyWhitespaceExcluded(t *testing.T) {
	diffmetrics, _ := CalculateDiffMetricsWhitespaceExcluded("https://github.com/andymeneely/git-churn", "f33d22b9b10a084ef494df3c9780d30c41d3f54d", "testdata/file.txt")
	got := fmt.Sprintf("%v", diffmetrics)
	expected := "&{testdata/file.txt 3 0 3 6 false false}"
	if got != expected {
		t.Errorf("DiffMetrics expected %q but got %q", expected, got)
	}
}
...
func TestFileDeletesOnlyWhitespaceExcluded(t *testing.T) {
	diffmetrics, _ := CalculateDiffMetricsWhitespaceExcluded("https://github.com/andymeneely/git-churn", "09e4b342693bf31bfb7cead1eb9b9fd59e3eef87", "testdata/file.txt")
	got := fmt.Sprintf("%v", diffmetrics)
	expected := "&{testdata/file.txt 0 1 6 5 false false}"
	if got != expected {
		t.Errorf("DiffMetrics expected %q but got %q", expected, got)
	}
}
...
func TestFileChangingLinesWhitespaceExcluded(t *testing.T) {
	diffmetrics, _ := CalculateDiffMetricsWhitespaceExcluded("https://github.com/andymeneely/git-churn", "00da33207bbb17a149d99301012006fbd86c80e4", "testdata/file.txt")
	got := fmt.Sprintf("%v", diffmetrics)
	expected := "&{testdata/file.txt 1 1 5 5 false false}"
	if got != expected {
		t.Errorf("DiffMetrics expected %q but got %q", expected, got)
	}
}
...
func TestFileDeleteWhitespaceExcluded(t *testing.T) {
	diffmetrics, _ := CalculateDiffMetricsWhitespaceExcluded("https://github.com/andymeneely/git-churn", "28b27020585be592df042c61ddab562665ce84cc", "testdata/file.txt")
	got, err := json.Marshal(diffmetrics)
	if err != nil {
		panic(err)
	}
	expected := "{\"File\":\"testdata/file.txt\",\"Insertions\":0,\"Deletions\":6,\"LinesBefore\":6,\"LinesAfter\":0,\"NewFile\":false,\"DeleteFile\":true}"
	if string(got) != expected {
		t.Errorf("DiffMetrics expected %q but got %q", expected, got)
	}
}
